...
const polygon = context.aimPolygon;
...
test('unblocked aim reaches the far baseline and stays inside sidelines',() => {
  ...
  assert.ok(points.every(p => p.x >= 7-1e-8 && p.x <= 93+1e-8 && p.y >= 10-1e-8 && p.y <=146));
});
...
test('a defender behind the ball does not block the target',() => {
  assert.equal(polygon(ball,[{x:50,y:170,radius:5}],bounds,70,10)[361].y,10);
});
test('opponent possession mirrors the shooting region toward home',() => {
  ...
  const reverse = polygon({x:50,y:54},[{x:50,y:115,radius:5}],bounds,130,190);
  forward.forEach((p,i) => { assert.ok(Math.abs(p.x-reverse[i].x)<1e-8); assert.ok(Math.abs(200-p.y-reverse[i].y)<1e-8); });
});
