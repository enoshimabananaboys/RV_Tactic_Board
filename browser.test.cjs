...
test('saved formations, responsive toolbar, and offline PWA',async () => {
  ...
  try {
    ...
    await page.locator('.help-content').evaluate(el => el.scrollTop=el.scrollHeight);
    assert.deepEqual(await page.locator('#close-help').boundingBox(),closeBefore);
    await page.locator('#help-dialog summary').click();
    assert.equal(await page.locator('#help-dialog details').getAttribute('open'),'');
    await page.keyboard.press('Control+z');
    assert.deepEqual(await positions(),original,'help does not change board');
    await page.screenshot({path:path.join(process.env.TEMP,'rv-help-mobile.png')});
    ...
    assert.equal(await page.locator('#tool-panel').isVisible(),true);
    assert.equal(await page.locator('#open-help').evaluate(el => el===document.activeElement),true);
    await page.locator('#open-help').click();
    ...
    await touch('touchStart',[one,two]);
    await touch('touchMove',[{...one,x:290},{...two,x:350}]);
    await page.waitForFunction(() => Number(document.querySelector('.court-wrap').style.getPropertyValue('--court-zoom'))<.6);
    await touch('touchEnd',[]);
    const fullFit = await page.locator('.court-scroll').evaluate(el => {const c=el.querySelector('#court').getBoundingClientRect();return c.width<=el.clientWidth && c.height<=el.clientHeight;});
    ...
    await page.locator('.court-scroll').evaluate(el => {el.scrollTop=0;el.scrollLeft=0;});
    await doubleTapEmpty();
    assert.ok(await page.locator('.court-wrap').evaluate(el => Number(el.style.getPropertyValue('--court-zoom'))<1));
    assert.equal(await page.locator('#arrow-lines line').count(),0,'double taps do not draw arrows');
    await checkBallSize();
    await setOrientation('portrait');
